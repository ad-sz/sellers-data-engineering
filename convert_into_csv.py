...
def sales_transactions_csv_processed(SALES_TRANSACTIONS_JSON_FILEPATH_OLD, SALES_TRANSACTIONS_CSV_FILEPATH_NEW, CUSTOMERS_CSV_FILEPATH_OLD, SELLERS_CSV_FILEPATH_OLD):
    ...
    # change all letters for small
    data_frame_sales_transactions_json = data_frame_sales_transactions_json.applymap(lambda x: x.lower() if type(x) == str else x)
...
def products_csv_processed(PRODUCTS_CSV_FILEPATH_OLD, PRODUCTS_CSV_FILEPATH_NEW):
    ...
    # change all letters for small
    data_frame_products_csv = data_frame_products_csv.applymap(lambda x: x.lower() if type(x) == str else x)
